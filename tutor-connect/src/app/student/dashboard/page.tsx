...
export default async function StudentDashboardPage() {
    ...
    // FIX: Strictly check if user ID exists to prevent Prisma error
    if (!session?.user?.id) {
        redirect('/login');
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            {/* Stats Cards - Original Colors & Material Icons */}
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">

                {/* Wallet Balance (Primary/Emerald Theme) */}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Link
                    href="/search"
                    className="rounded-xl bg-white p-6 shadow-sm border border-slate-200 hover:border-emerald-500 hover:shadow-md transition-all group"
                >
