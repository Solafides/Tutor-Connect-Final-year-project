...
export const bookingSchema = z.object({
    ...
    // Complex schedule array (JSON string)
    schedule: z.string().optional(), 
    
    // Shared / Colleague fields
    startDate: z.string().optional(),
    endDate: z.string().datetime().optional().or(z.string().optional()),
    packageDuration: z.string().optional(),
    daysPerWeek: z.number().min(1).max(7).optional(),
    selectedDays: z.array(z.enum(['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'])).min(1).optional(),
    startTime: z.string().min(1).optional(),
    endTime: z.string().min(1).optional(),
    timezone: z.string().min(1).optional(),
    hoursPerSession: z.number().min(1).optional(),
